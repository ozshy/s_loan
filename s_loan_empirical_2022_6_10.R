...
#
dir()
(fed_loans.df = read.csv("fed_loans_2022_6_10.csv" ))
names(fed_loans.df)
...
ggplot(m5.df, aes(x=year_start, y=total_fed_loans_dollar_millions/1000)) + geom_line(size=1.2, color="black") + geom_line( aes(x=year_start, y=fed_loans_undergrad/1000), size=1.2, color="black") + geom_line(aes(x=year_start, y=y_axis_2_scale*tuition_public), size=1.2, color="blue") + geom_line(aes(x=year_start, y=y_axis_2_scale*tuition_private), size=1.2, color="blue")  + scale_x_continuous(breaks = seq(1970, 2020, 5)) + scale_y_continuous(breaks = seq(0, 180, 10), name = "Federal student loans in 2020 Dollars (billions)" , sec.axis = sec_axis(trans = ~./y_axis_2_scale , name = "Total tuition, fees, room, and board (2019-2020 dollar prices)")) + theme(axis.text.y.right = element_text(color="blue"))  +annotate("text", x = 1976, y = 83, label = "Private college tuition", size = 7, color="blue") +annotate("text", x = 1976, y = 43, label = "Public college tuition", size = 7, color="blue") +annotate("text", x = 1975, y = 3, label = "Federal student loans", size = 7, color="black") +annotate("text", x = 2005, y = 26, label = "Federal loans (undergraduate only)", size = 7, color="black")  + theme(axis.text.x = element_text(size = 18, color = "black"), axis.text.y = element_text(size = 18, color = "black"), text = element_text(size = 20)) +ylab("Equilibrium tuition levels") + xlab("Academic year start") + theme(axis.title.y.right = element_text(vjust = 3)) + theme(plot.margin = margin(0.5,1,0.5,0.5, "cm")) + theme(axis.title.y.right = element_text(color="blue"))

# computing correlations for discussion of Figure 1
names(m5.df)
#
with(m5.df, cor(total_fed_loans_dollar_millions, tuition_private) )
cor(m5.df$total_fed_loans_dollar_millions, m5.df$tuition_private)
#
with(m5.df, cor(total_fed_loans_dollar_millions, tuition_public) )
cor(m5.df$total_fed_loans_dollar_millions, m5.df$tuition_public)
#
with(subset(m5.df, year_start > 1989), cor(total_fed_loans_dollar_millions, tuition_private) )
#
with(subset(m5.df, year_start > 1989), cor(total_fed_loans_dollar_millions, tuition_public) )
