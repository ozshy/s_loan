...
### Section 6: Equilibrium with maximizing colleges: equations (12)-(13) => Not used in paper. 
# Model parameters
(delta = 0.8)# discount factor
(r1 = 0.2) #interest on loans (over 4 years)
(r2 = 0.15) # reduced r (loans are more accessible)
...
# verify Assumption 2(a)
(1-delta)/delta# Hence,
r1 < (1-delta)/deltar2 < (1-delta)/delta

# equilibrium tuition: eq (12)(13)
(lam = delta*rho*(4*mua-mub-3*mun) -3*mun)
*
(ta1_numerator.vec = 2*c.vec*delta*rho*(mua-mub)*(1-r1*delta-delta) + 4*delta^2*rho^2*(mua-mub)*(mua-mun) - delta*rho*(mua*(4*mun+3) - mun*(4*mub+1))+3*mun)
(ta1.vec = ta1_numerator.vec/(2*lam))
# verify loan cap is binding
ta1.vec > c.vec
#
(tb1_numerator.vec = 4*c.vec*delta*rho*(mua-mub)*(1-r1*delta-delta) + 2*delta^2*rho^2*(mua-mub)*(mub-mun) - delta*rho*(2*mua*(mun+1) + mub*(1-2*mun)-3*mun)+3*mun)
(tb1.vec = tb1_numerator.vec/(2*lam))
# verify loan cap is binding
tb1.vec > c.vec
#
(ta2_numerator.vec = 2*c.vec*delta*rho*(mua-mub)*(1-r2*delta-delta) + 4*delta^2*rho^2*(mua-mub)*(mua-mun) - delta*rho*(mua*(4*mun+3) - mun*(4*mub+1))+3*mun)
(ta2.vec = ta2_numerator.vec/(2*lam))
# verify loan cap is binding
ta2.vec > c.vec
#
(tb2_numerator.vec = 4*c.vec*delta*rho*(mua-mub)*(1-r2*delta-delta) + 2*delta^2*rho^2*(mua-mub)*(mub-mun) - delta*rho*(2*mua*(mun+1) + mub*(1-2*mun)-3*mun)+3*mun)
(tb2.vec = tb2_numerator.vec/(2*lam))
# verify loan cap is binding
tb2.vec > c.vec
#
...
# Figure 4: plot teql [ta, tb] (maximizing colleges) => NOT used in paper
